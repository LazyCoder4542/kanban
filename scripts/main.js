...
class myApp {
    ...
    generateMenuNav() {
        ...
        <ul id="nav-desktop"></ul>
          ...
          <span>&plus; Create New Board</span>
          </span>
        </div>
        </div>
        <div class="theme">
        ...
        if (this.mql.matches) {
            ...
            console.log(document.querySelector(".container").classList.contains("side-bar-opened"));
            console.log("svdjkbksbdv");
            if (document.querySelector(".container").classList.contains("side-bar-opened")) {
                console.log("fried");
                listener();
            }
            document.querySelector('#board-name').addEventListener('click', listener, { signal: this.menuController.signal });
        }
        else {
...
const app = () => __awaiter(this, void 0, void 0, function* () {
    let data = yield fetch('./data/dev-data.json').then(res => res.json());
